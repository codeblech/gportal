...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (username: string, password: string, captcha: string) => {
    try {
      ...
      const loginPageResponse = await fetch('/api/glbajaj/Login');
      ...
      const viewstateGenerator = loginDoc.querySelector('input[name="__VIEWSTATEGENERATOR"]')?.getAttribute('value') || '';
      ...
      console.log('Extracted payload keys:', { viewstate: viewstate.substring(0, 20) + '...', viewstateGenerator, eventValidation: eventValidation.substring(0, 20) + '...', captchaPayloadKey });
      ...
      // Perform login
      const loginResponse = await fetch('/api/glbajaj/Login', {
        method: 'POST',
        headers: {
      ...
      const attendancePageResponse = await fetch('/api/glbajaj/Student/TodayAttendence', {
        credentials: 'include',
      });
      ...
      const payload: PayloadData = {
        ...
        __VIEWSTATEGENERATOR: attendanceDoc.querySelector('input[name="__VIEWSTATEGENERATOR"]')?.getAttribute('value') || '',
        __EVENTVALIDATION: attendanceDoc.querySelector('input[name="__EVENTVALIDATION"]')?.getAttribute('value') || '',
        hdnStudentId: attendanceDoc.querySelector('input[name="ctl00$ctl00$MCPH1$SCPH$hdnStudentId"]')?.getAttribute('value') || '',
      };
  ...
  const logout = async () => {
    try {
      ...
      const homePageResponse = await fetch('/api/glbajaj/home', {
        credentials: 'include',
      });
      ...
      const viewstateGenerator = homeDoc.querySelector('input[name="__VIEWSTATEGENERATOR"]')?.getAttribute('value') || '';
      ...
      const hdncollegenewsalerts = homeDoc.querySelector('input[name="ctl00$hdncollegenewsalerts"]')?.getAttribute('value') || '';
      const hdnsystemalerts = homeDoc.querySelector('input[name="ctl00$hdnsystemalerts"]')?.getAttribute('value') || '';
      const hdnattendancealerts = homeDoc.querySelector('input[name="ctl00$hdnattendancealerts"]')?.getAttribute('value') || '';
      const hdnlibraryalerts = homeDoc.querySelector('input[name="ctl00$hdnlibraryalerts"]')?.getAttribute('value') || '';
      const hdnCollege = homeDoc.querySelector('input[name="ctl00$hdnCollege"]')?.getAttribute('value') || '';
      const hdnForSchoolMaster = homeDoc.querySelector('input[name="ctl00$hdnForSchoolMaster"]')?.getAttribute('value') || '';
  ...
  return (
    <AuthContext.Provider value={{ isAuthenticated, login, logout, payloadData }}>
      {children}
